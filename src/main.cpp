...
ostream &operator << (ostream &out, const ground_truth &gt) {
	out << "(" << gt.x << ", " << gt.y << ", " << gt.theta << ")";
}
...
int main() {
	...
	for (int i = 0; i < num_time_steps; ++i) {
		...
		if (!pf.initialized()) {
			...
			cout << "Initialized particle filter" << endl;
		}
		else {
		...
		cout << "Current predicted position: " << pf.state << endl;
		cout << "Current ground truth position: " << gt[i] << endl;
