 ...
 *
 * @param a x and y coordinates of first point
 * @param b x and y coordinates of second point
 *
 * @return Euclidean distance between two 2D points
 */
template<class A, class B> double dist(const A &a, const B &b) {
	double dx = a.x - b.x;
	double dy = a.y - b.y;
...
inline double pdf(const std::vector<LandmarkObs> &mapped, const std::vector<LandmarkObs> &predicted, double std_landmark[]) {
	double sx = std_landmark[0];
	double sy = std_landmark[1];
	double eta = sqrt(2 * M_PI * pow2(sx) * pow2(sy));

	double w = 1.0;
	for (int i = 0, n = mapped.size(); i < n; ++i) {
		const LandmarkObs &z = mapped[i];
		const LandmarkObs &u = predicted[i];
		double dx = (z.x - u.x) / sx;
		double dy = (z.y - u.y) / sy;

		w *= exp(-0.5 * (pow2(dx) + pow2(dy))) / eta;
	}
